...
let projectsStore = [
  {
    ...
    description: 'An intelligent chatbot powered by machine learning to assist users with common queries.',
    problemStatement: 'Users struggle to find quick answers to common questions',
...
let teamMembersStore = [
  { id: 1, projectId: 1, userId: 'user-1', name: 'Alice Johnson', email: 'alice@example.com', role: 'Full-stack Developer', isLeader: true, createdAt: '2025-01-10T08:00:00Z', updatedAt: '2025-01-10T08:00:00Z' },
  { id: 2, projectId: 1, userId: null, name: 'Bob Smith', email: 'bob@example.com', role: 'ML Engineer', isLeader: false, createdAt: '2025-01-10T08:05:00Z', updatedAt: '2025-01-10T08:05:00Z' },
  { id: 3, projectId: 2, userId: 'user-2', name: 'Charlie Brown', email: 'charlie@example.com', role: 'Frontend Developer', isLeader: true, createdAt: '2025-01-12T10:00:00Z', updatedAt: '2025-01-12T10:00:00Z' },
  { id: 4, projectId: 2, userId: null, name: 'Diana Prince', email: 'diana@example.com', role: 'Backend Developer', isLeader: false, createdAt: '2025-01-12T10:05:00Z', updatedAt: '2025-01-12T10:05:00Z' },
  { id: 5, projectId: 3, userId: 'user-3', name: 'Eve Adams', email: 'eve@example.com', role: 'Data Scientist', isLeader: true, createdAt: '2025-01-11T14:00:00Z', updatedAt: '2025-01-11T14:00:00Z' },
];

let judgesStore = [
  { id: 1, userId: 'judge-1', name: 'Dr. Sarah Lee', email: 'sarah.lee@gdg.dev', bio: 'AI/ML expert with 10 years experience', expertise: ['AI/ML', 'Data Science'], isActive: true, createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 2, userId: 'judge-2', name: 'Mark Wilson', email: 'mark.wilson@gdg.dev', bio: 'Full-stack developer and cloud architect', expertise: ['Web Development', 'Cloud'], isActive: true, createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
];
...
let evaluationsStore = [
  {
    ...
    presentationScore: 8,
    totalScore: 43,
    feedback: 'Excellent project with great potential. The AI integration is impressive.',
...
let userRolesStore = [
  { id: 1, userId: 'user-1', role: 'participant', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 2, userId: 'user-2', role: 'participant', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 3, userId: 'user-3', role: 'participant', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 4, userId: 'judge-1', role: 'judge', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 5, userId: 'judge-2', role: 'judge', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 6, userId: 'organizer-1', role: 'organizer', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
  { id: 7, userId: 'admin-1', role: 'admin', createdAt: '2025-01-05T00:00:00Z', updatedAt: '2025-01-05T00:00:00Z' },
];
...
export const handlers = [
  ...
  http.put('/api/projects/:id', async ({ params, request }) => {
    const projectIndex = projectsStore.findIndex(p => p.id === Number(params.id));
    if (projectIndex === -1) {
      return HttpResponse.json({ error: 'Project not found' }, { status: 404 });
    }
    ...
    const updated = {
      ...
      ...body,
      updatedAt: new Date().toISOString(),
      submittedAt: body.status === 'submitted' && !currentProject?.submittedAt
  ...
  http.get('/api/projects/:id/team', ({ params }) => {
    const members = teamMembersStore.filter(m => m.projectId === Number(params.id));
  ...
  http.delete('/api/team-members/:id', ({ params }) => {
    const memberIndex = teamMembersStore.findIndex(m => m.id === Number(params.id));
    if (memberIndex === -1) {
      return HttpResponse.json({ error: 'Team member not found' }, { status: 404 });
    }
    teamMembersStore.splice(memberIndex, 1);
    return HttpResponse.json({ message: 'Team member removed' }, { status: 200 });
  }),
  ...
  http.post('/api/evaluations', async ({ request }) => {
    ...
    if (existing) {
      return HttpResponse.json({ error: 'Evaluation already exists for this judge and project' }, { status: 400 });
    }
  ...
  http.put('/api/evaluations/:id', async ({ params, request }) => {
    const evalIndex = evaluationsStore.findIndex(e => e.id === Number(params.id));
    if (evalIndex === -1) {
      return HttpResponse.json({ error: 'Evaluation not found' }, { status: 404 });
    }
  ...
  http.get('/api/users/:userId/role', ({ params }) => {
    ...
    if (!userRole) {
      return HttpResponse.json({ error: 'User role not found' }, { status: 404 });
    }
  ...
  http.put('/api/users/:userId/role', async ({ params, request }) => {
    const roleIndex = userRolesStore.findIndex(ur => ur.userId === params.userId);
    if (roleIndex === -1) {
      return HttpResponse.json({ error: 'User role not found' }, { status: 404 });
    }
  ...
  http.get('/api/leaderboard', () => {
    ...
    const leaderboard = Object.entries(projectScores)
      .map(([projectId, scores]) => {
        ...
        const teamLeader = teamMembersStore.find(m => m.projectId === Number(projectId) && m.isLeader);
  ...
  http.get('/api/organizer/stats', () => {
    const stats = {
      ...
      submittedProjects: projectsStore.filter(p => p.status === 'submitted' || p.status === 'reviewed').length,
      draftProjects: projectsStore.filter(p => p.status === 'draft').length,
      reviewedProjects: projectsStore.filter(p => p.status === 'reviewed').length,
      totalParticipants: new Set(teamMembersStore.map(m => m.userId).filter(Boolean)).size,
      ...
      finalizedEvaluations: evaluationsStore.filter(e => e.isFinalized).length,
    };
